...
T2S_FSM_State0: ; Checks for start signal (T2S_FSM_Start==1)
	cjne a, #0, T2S_FSM_State0
	jnb T2S_FSM_Start, T2S_FSM_State0_Done
	; Check if degrees is larger than 200
	clr c
	mov a, degrees
	subb a, #200
	jnc degrees_gt_200
	; Check if degrees is larger than 100, less than 200
	clr c
	mov a, degrees
	subb a, #100
	jnc degrees_gt_100
	; Go to state to check if degrees < 20
	mov T2S_FSM_state, #5
...
T2S_FSM_State1: ; Plays "100" audio clip
	cjne a, #1, T2S_FSM_State3
	mov a, #28 ; Index 28 has "100" audio clip
	lcall Play_Sound_Using_Index
	mov T2S_FSM_state, #3
	ret

T2S_FSM_State2: ; Plays "200" audio clip
	cjne a, #2, T2S_FSM_State4
	mov a, #29 ; Index 29 has "200" audio clip
	lcall Play_Sound_Using_Index
	mov T2S_FSM_state, #4
	ret

T2S_FSM_State3: ; Stay in this state until State1 finishes playing
	cjne a, #3, degrees_gt_20_q
	jb TMOD20, T2S_FSM_State3_Done
	; Check if degrees % 100 > 20
	clr c
	mov a, degrees
	mov b, #10
	div ab
...
T2S_FSM_State4: ; Stay in this state until State2 finishes playing
	cjne a, #4, T2S_FSM_State10
	jb TMOD20, T2S_FSM_State4_Done
	mov T2S_FSM_state, #10
T2S_FSM_State4_Done:
	ret

T2S_FSM_State5: ; Checks if degrees < 20 or > 20
	clr c
	mov a, degrees
	subb a, #20
	jnc degrees_gt_20
	mov T2S_FSM_state, #7
	sjmp T2S_FSM_State5_Done
degrees_gt_20:
	mov T2S_FSM_state, #6
T2S_FSM_State5_Done:
	ret

T2S_FSM_State6: ; Plays "[degrees/10]" audio clip, ex) 95 -> plays "90" audio clip
	cjne a, #6, T2S_FSM_State8
	mov a, degrees
	mov b, #10
	div ab
	add a, #18
...
T2S_FSM_State7: ; Plays "[degrees]" audio clip, ex) for degrees < 20
	cjne a, #7, T2S_FSM_State11
	mov a, degrees
	lcall Play_Sound_Using_Index
	mov T2S_FSM_state, #11
	ret

T2S_FSM_State8: ; Stay in this state until State6 finishes playing
	cjne a, #8, T2S_FSM_State12
	jb TMOD20, T2S_FSM_State8_Done
	mov T2S_FSM_state, #12
T2S_FSM_State8_Done:
	ret

T2S_FSM_State9: ; Plays "[degrees % 100]" audio clip, ex) 100 < degrees < 200
	cjne a, #9, T2S_FSM_State15
	mov a, degrees
	mov b, #10
	div ab
	lcall Play_Sound_Using_Index
...
T2S_FSM_State10: ; Plays "[degrees % 100] / 10" audio clip, ex) 223 -> plays "20"
	cjne a, #10, T2S_FSM_State13
	mov a, degrees
	mov b, #100
	div ab
	mov b, #10
	div ab
	add a, #18 ; ex) [223 % 100] / 10 = 2.3 => 2, 2+18 = 20 <- Index # for "20" audio
	lcall Play_Sound_Using_Index
	mov T2S_FSM_state, #13
	ret

T2S_FSM_State11: ; Stay in this state until State7 finishes playing
	cjne a, #11, T2S_FSM_State18
	jb TMOD20, T2S_FSM_State11_Done
	mov T2S_FSM_state, #18
T2S_FSM_State11_Done:
	ret

T2S_FSM_State12: ; Play "[degrees % 10]" audio clip, ex) 86 -> plays "6"
	cjne a, #12, T2S_FSM_State14
	mov a, degrees
	mov b, #10
	div ab
	lcall Play_Sound_Using_Index
...
T2S_FSM_State13: ; Stay in this state until State10 finishes playing
	cjne a, #13, T2S_FSM_State16
	jb TMOD20, T2S_FSM_State13_Done
	mov T2S_FSM_state, #16
T2S_FSM_State13_Done:
	ret

T2S_FSM_State14: ; Stay in this state until State12 finishes playing
	cjne a, #14, T2S_FSM_State18
	jb TMOD20, T2S_FSM_State14_Done
	mov T2S_FSM_state, #18
T2S_FSM_State14_Done:
	ret

T2S_FSM_State15: ; Stay in this state until State9 finishes playing
	cjne a, #15, T2S_FSM_State18
	jb TMOD20, T2S_FSM_State15_Done
	mov T2S_FSM_state, #18
T2S_FSM_State15_Done:
	ret

T2S_FSM_State16: ; Plays "[degrees % 100] % 10" audio clip, ex) 167 -> plays "7" audio clip
...
T2S_FSM_State17: ; Stay in this state until State16 finishes playing
	cjne a, #17, T2S_FSM_State18
	jb TMOD20, T2S_FSM_State17_Done
	mov T2S_FSM_state, #18
T2S_FSM_State17_Done:
	ret

T2S_FSM_State18: ; Plays "degrees celsius" audio clip
	cjne a, #18, T2S_FSM_State19
	mov a, #38 ; Index 38 has "degrees celsius" audio clip
	lcall Play_Sound_Using_Index
	mov T2S_FSM_state, #19
	ret

T2S_FSM_State19: ; Stay in this state until State18 finishes playing
	cjne a, #19, T2S_FSM_Error
	jb TMOD20, T2S_FSM_State19_Done
	clr T2S_FSM_Start
	mov T2S_FSM_state, #0
T2S_FSM_State19_Done:
	ret
