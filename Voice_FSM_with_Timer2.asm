...
Temperature_Counter:
	mov a, Time_counter
	cjne a, #5, Increment ; Compares to see if 5 sec have passed
	mov a, #0x00
	setb T2S_FSM_Start ; Sets bit to 1 so FSM starts
Increment:
	mov a, Time_counter
	add a, #0x01
	da a
...
; T2S_FSM_state - state number for the temperature reading
; Reflow oven state reading - include with LCD display
...
T2S_FSM_State13:
    ...
    ret
;------------------------------------------------------14--------------------------------------------
T2S_FSM_State14: ;Stay in this state until State1 finishes playing
	cjne a, #14, T2S_FSM_State0
